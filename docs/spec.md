...
- **RF-69 (Ubiquitous):** The portal médico público DEBE leer el catálogo de impulso exclusivamente desde la vista `inventario_impulso_publico` (columnas `odoo_id, product_name, stock, marca`); el rol `anon` NO DEBE tener privilegios sobre `inventario_local`. En `citas_medicas`, `anon` solo puede leer y actualizar la columna `estado`.
