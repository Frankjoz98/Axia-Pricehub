...
- **RF-7 (Ubiquitous):** The sistema DEBE cruzar los datos de `ventas_historicas` con `inventario_local` mediante el índice único `buildInventarioIndex()` de `src/lib/odoo.ts` (odoo_id normalizado con fallback por nombre sin `[REF]`). Todos los módulos (métricas, stock muerto, alertas, agenda, reabastecimiento) DEBEN usar ese índice; ninguno cruza por nombre por su cuenta. WHEN un producto en ventas cruza con `inventario_local`, THEN el sistema DEBE usar el `product_name` del inventario como nombre canónico para toda agrupación y visualización de métricas (excepto en la vista Lupa, que muestra datos crudos para auditoría).
...
- **RF-12 (Ubiquitous):** The sistema DEBE proveer una vista de Analíticas de Compras (pestaña "Analíticas" del Panel de Órdenes) que muestre la Varianza Histórica y el Ratio de Compras vs. Ventas basado en las órdenes conciliadas. Para el Flujo Comercial del Dashboard, una orden cuenta como "comprada" si está conciliada o si tiene recepción registrada (`conciliado || status !== 'Pendiente'`).
...
- **RF-15 (Ubiquitous):** The sistema DEBE generar un reporte de "Productos Estancados" (Dead Stock), identificando productos con stock mayor a cero pero sin unidades vendidas en el periodo evaluado (cruce por RF-7), ordenados por capital congelado (Costo Total).
...
- **RF-50 (Ubiquitous):** WHEN el sistema agrupa ventas para la gráfica de tendencias temporales, THEN DEBE asociar las transacciones al día operativo (`businessDate`) de su sesión de caja, calculado en HORA LOCAL a partir del punto medio de la sesión, garantizando que las ventas nocturnas no se fragmenten en días contables distintos. Los turnos son únicos para todo el sistema: Mañana 06:00–13:59, Tarde 14:00–21:59, Noche 22:00–05:59; una venta sin sesión se clasifica con la misma regla.
- **RF-50b (Ubiquitous):** The sistema NUNCA DEBE derivar "el día" de una fecha con `toISOString()` ni parsear `YYYY-MM-DD` con `new Date()`: toda conversión pasa por `src/lib/dates.ts` (`todayYMD`, `toLocalYMD`, `parseLocalYMD`, `daysUntil`), porque la farmacia opera en UTC-6 y ambos métodos corren el día después de las 18:00.
...
- **RF-52 (Event-Driven):** WHEN el usuario selecciona un quick filter (Hoy, Esta Semana, Mes Actual, Histórico), THEN el sistema DEBE calcular dinámicamente los límites en hora local y filtrar las ventas por su día operativo (RF-50) antes de derivar cualquier métrica: las sesiones se calculan siempre sobre el histórico completo y una sesión entra o sale del rango completa, nunca fragmentada (así el total de Cierres cuadra con el Z-Report).
...
### Módulo de Presupuesto y Reabastecimiento
- **RF-72 (Ubiquitous):** The presupuesto semanal (`weeklySalesEfectivo` en `AppContext`) es la ÚNICA fuente para el header, el Dashboard y el carrito: promedio de las últimas 4 semanas con datos dividiendo entre las semanas realmente cubiertas; si no hay ventas se usa el valor manual de respaldo.
- **RF-73 (Ubiquitous):** WHEN Axia AI calcula sugerencias de reabastecimiento, THEN DEBE estimar la venta diaria sobre una ventana de 30 días con datos y los días realmente cubiertos (nunca una constante), cruzar por RF-7, excluir ofertas con precio 0 ("Consultar") al elegir la mejor opción, y agregar al carrito la cantidad sugerida por defecto.
- **RF-74 (Event-Driven):** WHEN el importador de catálogo recibe una fila sin `precio_neto`, THEN DEBE derivarlo como `precio_base × (1 − descuento_porcentaje/100)`.
- **RF-75 (Event-Driven):** WHEN el usuario reabre una orden al carrito, THEN el sistema DEBE rechazarlo si la orden está conciliada, fusionar los ítems con los existentes en el carrito, y mantener la orden en el historial si algún producto ya no existe en el catálogo (avisando cuántos faltaron).
- **RF-76 (Event-Driven):** WHEN el importador de ventas encuentra una fila sin fecha válida o con cantidades no numéricas, THEN DEBE descartar la fila (nunca insertar fechas 1970 ni `NaN`).

### Módulo de Seguridad y Roles
- **RF-67 (Ubiquitous):** The sistema DEBE resolver el rol de cada usuario (`admin` | `caja`) desde la tabla `perfiles` mediante la función `auth_rol()`. Las tablas sensibles (`productos`, `ventas_historicas`, `inventario_local`, `configuracion`, `ordenes_compra`, `proveedores`, `facturas_compra`, `fichas_tecnicas`, `agenda_eventos`, `bitacora_avances`, `metas`) DEBEN tener políticas RLS que solo permitan acceso al rol `admin`; el aislamiento en el cliente (`App.tsx`) es únicamente visual.
...
- **RF-69 (Ubiquitous):** The portal médico público DEBE acceder a datos únicamente mediante las funciones RPC `portal_catalogo_impulso`, `portal_citas` y `portal_actualizar_cita`, que exigen el token de `configuracion.portal_token` (enlace `/portal-medico?k=<token>`). El rol `anon` NO DEBE tener privilegios sobre ninguna tabla ni vista: los nombres de pacientes son datos personales y nunca se exponen sin token. El catálogo de impulso devuelve solo `odoo_id, product_name, stock, marca` (sin precio ni costo). El administrador ve y copia el enlace desde Ajustes; para revocar el acceso se rota `portal_token`. WHEN el portal carga con `?k=`, THEN DEBE guardar el token en `sessionStorage` y retirarlo de la URL (`replaceState`) para que no quede en historial ni en `Referer`; el token viaja siempre en el cuerpo POST de las RPC y la ruta se sirve con `Referrer-Policy: no-referrer`.
